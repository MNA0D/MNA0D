...
function DataTable({ infectionData }) {
  ...
  const handleRowClick = (id) => {
    window.location.href = `http://localhost:3000/features?sheep=${id}`;
  };
      ...
      <InputGroup className="mb-3">
        ...
        <Dropdown>
          ...
          <Dropdown.Menu>
            ...
            <Dropdown.Item onClick={() => handleItemsPerPageChange(30)}>
              30
            </Dropdown.Item>
      ...
      <Table striped bordered hover>
        <thead>
          <tr>
            ...
            <th>
              <Button
                variant="link"
                onClick={() => handleSort("infectionDate")}
              >
                Date d'Infection{" "}
                {sortConfig.key === "infectionDate"
                  ? sortConfig.direction === "ascending"
                    ? "↑"
                    : "↓"
                  : ""}
        ...
        <tbody>
          {currentItems.map((data, index) => (
            <tr
              key={index}
              style={{ backgroundColor: data.active ? "white" : "lightgrey" }}
